-- Fix log_attendance_with_deduction to handle players with multiple active subscriptions
-- by targeting only the most recent active subscription (ORDER BY created_at DESC LIMIT 1).
-- Also sets sessions_remaining = 0 for single-session packages (sessions_total = 1).
...
BEGIN
  ...
  -- If present, deduct session from the most recent active subscription
  IF p_status = 'present' THEN
    ...
    IF v_sub_id IS NOT NULL THEN
      -- For single-session packages (sessions_total = 1), set remaining to 0
      -- For multi-session packages, decrement by 1
