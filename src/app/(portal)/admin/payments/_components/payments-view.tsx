...
import { PaymentsPageSkeleton, PaymentsInlineSkeleton } from "./skeleton";
import { PaymentsFilters } from "./filters";
import { PaymentsTableView } from "./table";
import { RejectModal, ConfirmDeleteDrawer, ScreenshotLightbox } from "./modals";
import { PaymentDrawer } from "./payment-drawer";
import { NewPaymentDrawer } from "./new-payment-drawer";
...
interface FreeGrant {
  ...
  profiles: PaymentRow["profiles"];
  subscriptions: PaymentRow["subscriptions"];
}

/** What a promo took off this payment. `amount` is already net of it, so this is for display only. */
...
export function PaymentsView() {
  ...
  useEffect(() => {
    // Start from the live params so keys we don't own (e.g. the Finances `tab`)
    // survive a filter change instead of being wiped by the rebuild.
    const params = new URLSearchParams(searchParams.toString());
    for (const k of ["q","statusFilter","package","month","method","type","sort","dir","page","size"]) params.delete(k);
  ...
  async function fetchPayments() {
    const { data } = await supabase
      .from("payments")
      .select("*, profiles!payments_player_id_fkey(first_name, last_name, phone), subscriptions!payments_subscription_id_fkey(start_date, end_date, package_id, packages(id, name)), promo_codes!payments_promo_code_id_fkey(code), promo_code_uses(discount_amount)")
      .order("created_at", { ascending: false })
      .limit(5000);
    // A 100%-discount subscription never creates a payment, so the giveaway would be
    // invisible here. Surface those as read-only rows built from the recorded promo use.
    const { data: grants } = await supabase
      .from("promo_code_uses")
      .select("id, discount_amount, created_at, player_id, subscription_id, promo_codes(code), profiles(first_name, last_name, phone), subscriptions(start_date, end_date, package_id, packages(id, name))")
      .is("payment_id", null);

    const grantRows: PaymentRow[] = ((grants || []) as unknown as FreeGrant[]).map((g) => ({
      id: `free:${g.id}`,
      ...
      promo_code_id: null,
      promo_codes: g.promo_codes,
      promo_code_uses: [{ discount_amount: g.discount_amount }],
      is_free_grant: true,
    }));

    const rows = [...((data || []) as unknown as PaymentRow[]), ...grantRows]
  ...
  // Selection helpers
  // Free grants have no payments row, so they must never enter a bulk action
  const pageIds = paginatedPayments.filter((p) => !p.is_free_grant).map((p) => p.id);
          ...
          <button
            onClick={async () => {
              ...
              const rows = filteredPayments.map((p) => ({
                ...
                "Amount (EGP)": p.amount,
                "Promo Code": p.promo_codes?.code || "",
                "Discount (EGP)": discountOf(p) || "",
                Method: p.method,
                Status: p.status,
              }));
      ...
      <div className="flex-1">
        {loading ? (
          ...
          <PaymentsTableView
            ...
            totalDiscount={filteredPayments.reduce((sum, p) => sum + discountOf(p), 0)}
          />
        )}
      </div>
